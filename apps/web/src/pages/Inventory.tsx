...
interface Movement { id: string; quantity: string; reason: string; reference: string | null; createdAt: string; }
...
export default function Inventory() {
  ...
  const lookupMovements = async () => {
    if (!companyId) { setLookupError("No company is assigned to your account yet."); return; }
    setLookupError(null);
    setMovements(null);
    try {
      const rows = await apiGet<Movement[]>(`/companies/${companyId}/inventory/movements?warehouseId=${encodeURIComponent(lookupWarehouseId)}&productId=${encodeURIComponent(lookupProductId)}`);
      setMovements(rows);
    } catch (e) {
  ...
  const submitAdjustment = async () => {
    ...
    try {
      const adjustment = await apiPost(`/companies/${companyId}/inventory/adjustments`, { warehouseId, productId, countedQuantity, reason });
  ...
  return (
    <div>
      ...
      {lookupError && <p className="state">Could not reach the API ({lookupError}).</p>}
      {movements && (
        <table style={{ marginBottom: 24, maxWidth: 600 }}>
          <thead><tr><th>Quantity</th><th>Reason</th><th>Reference</th><th>Date</th></tr></thead>
          <tbody>
            {movements.map((m) => (
              <tr key={m.id}><td>{m.quantity}</td><td>{m.reason}</td><td>{m.reference ?? "—"}</td><td>{new Date(m.createdAt).toLocaleString()}</td></tr>
            ))}
          </tbody>
        </table>
      )}
      ...
      <div style={{ display: "grid", gap: 10, maxWidth: 420, marginBottom: 12 }}>
        ...
        <input type="number" placeholder="Counted quantity" value={countedQuantity} onChange={(e) => setCountedQuantity(Number(e.target.value))} />
